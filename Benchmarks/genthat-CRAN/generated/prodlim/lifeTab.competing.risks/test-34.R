...
function_to_run <- function() {
    ...
    stats <- list(c("cuminc", "0"), c("se.cuminc", "0"), c("lower", "0"), c("upper", "0"))
    showTime <- TRUE
    percent <- FALSE
    ...
    intervals <- FALSE
    cause <- "1"
    prodlim:::lifeTab.competing.risks(object = object, times = times, cause = cause, newdata = X, stats = stats, intervals = intervals,
        percent = percent, showTime = showTime)
}
