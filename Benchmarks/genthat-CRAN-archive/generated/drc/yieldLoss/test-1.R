...
genthat_extracted_call <- function() {
    ...
    MM.3 <- drc::MM.3
    met.mm.m1 <- structure(list(varParm = NULL, fit = list(par = c(1451.95618185924, 1736.14112096408, 1868.51673616717, 
        0.0389461639125828, 0.111041007890867), value = 496.652084397528, counts = structure(c(34L, 22L), names = c("function", 
        "gradient")), convergence = TRUE, message = NULL, hessian = structure(c(5.07741428566062, 1.50436601764241, 1.81860835428528, 
        -3501.5968594827, -3439.5616691896, 1.50436601764241, 4.19186293335743, -7.98031329759181e-15, -7475.56132461686, 
        0, 1.81860835428528, -7.98031329759181e-15, 2.08477403712792, -2.96764485608024e-10, -3382.78802827323, -3501.5968594827, 
        -7475.56132461686, -2.96764485608024e-10, 16011521.7638331, 2.96764485608024e-05, -3439.5616691896, 0, -3382.78802827323, 
        2.96764485608024e-05, 5883450.90106452), .Dim = c(5L, 5L)), ovalue = 0.000496652084397528), curve = list(genthat::with_env(function(dose) {
        if (is.vector(dose)) {
            lenPts <- length(dose)
        } else {
            lenPts <- nrow(dose)
    ...
    drc:::yieldLoss(object = met.mm.m1, interval = "as")
}
