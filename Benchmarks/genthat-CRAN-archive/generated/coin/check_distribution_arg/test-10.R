...
function_to_run <- function() {
    args <- list(teststat = "quadratic", distribution = "exact", alternative = c("two.sided", "less", "greater"), xtrafo = genthat::with_env(function(data, 
    ...
    }, env = getNamespace("coin")), ytrafo = genthat::with_env(function(data, numeric_trafo = id_trafo, factor_trafo = f_trafo, 
        ordered_trafo = of_trafo, surv_trafo = logrank_trafo, var_trafo = NULL, block = NULL) {
        `__genthat_captured_seed` <- get(".Random.seed", envir = globalenv())
        ""
        on.exit({
            if (.Internal(getOption("genthat.tracing"))) {
    ...
    }, env = getNamespace("coin")), scores = NULL, check = NULL)
    coin:::check_distribution_arg(distribution = args$distribution)
}
