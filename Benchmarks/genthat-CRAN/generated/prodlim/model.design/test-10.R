library(prodlim)

function_to_run <- function() {
    unspecialsDesign <- FALSE
    Terms <- Hist(time, event) ~ 1
    specialsFactor <- TRUE
    specialsDesign <- FALSE
    mm <- structure(list(time = c(5.7248465289322, 0.597364679005719, 5.32834369210633, 4.11975186868313, 2.58908125548376,
        2.34600241418988, 3.37065111669117, 1.82132226826408, 2.4465502378103, 0.836736115633699, 1.21539126795391, 6.19802755334038,
        2.20688663795295, 4.48477476990214, 2.28403441657353), event = c(1, 2, 0, 0, 1, 2, 1, 1, 0, 1, 1, 2, 1, 0, 1)), row.names = c(NA,
        15L), class = "data.frame")
    dropIntercept <- TRUE
    prodlim:::model.design(terms = Terms, data = mm, dropIntercept = dropIntercept, maxOrder = 1, unspecialsDesign = unspecialsDesign,
        specialsFactor = specialsFactor, specialsDesign = specialsDesign)
}




