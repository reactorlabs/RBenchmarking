...
function_to_run <- function() {
    ...
    prodlim:::EventHistory.frame(formula = Hist(time, status) ~ prop(X1) + X2 + cluster(X3) + X4, data = dsurv, specials = c("prop",
        "cluster"), specialsDesign = TRUE, stripSpecials = c("prop", "cluster"))
}
