...
function_to_run <- function() {
    ...
    x <- structure(list(Sepal.Length = c(5.1, 4.9, 4.7, 4.6, 5, 5.4, 4.6, 5, 4.4, 4.9, 5.4, 4.8, 4.8, 4.3, 5.8, 5.7, 5.4, 
        ...
        -150L))
    weights <- NULL
    trControl <- list(method = "cv", number = 10, repeats = NA, search = "grid", p = 0.75, initialWindow = NULL, horizon = 1, 
    ...
    trainInfo <- list(loop = structure(list(k = c(5L, 7L, 9L, 11L, 13L, 15L, 17L, 19L, 21L, 23L)), row.names = c(NA, 10L), 
        class = "data.frame"))
    ...
    classLevels <- structure(c("setosa", "versicolor", "virginica"), ordered = FALSE)
    caret:::nominalTrainWorkflow(x = x, y = y, wts = weights, info = trainInfo, method = models, ppOpts = preProcess, ctrl = trControl, 
        lev = classLevels)
}
